...
클라이언트로부터 Request Header의 Authorization 필드로
Bearer 토큰을 포함하여 요청을 받으면 해당 토큰의 작성자 (subject)에 해당하는 사용자 정보를 
반환합니다.
...
###### Example
```bash
curl -v -X GET "http://localhost:4000/api/vi/user/" \
...
클라이언트로부터 Request Header의 Authorization 필드로 Bearer 토큰을 
포함하여 검색어를 입력받고 요청을 보내면 작성일 기준 내림차순으로 제목에 해당 검색어가 포함된 게시물 리스트를 반환합니다. 만약 불러오기에 실패하면 실패처리를 합니다. 인가 실패, 데이터베이스 에러가 발생할 수 있습니다.
...
| name | type | description | required |
|---|:---:|:---:|:---:|
| searchWord | String | 검색어 | 0 |

###### Example
```bash
curl -v -X GET POST "http://localhost:4000/api/vi/board/list/{searchWord}" \
 -H "Authorization: Bearer {JWT}"
```
...
클라이언트로부터 Request Header의 Authorization 필드로 Bearer 토큰을 
포함하여 접수번호를 입력받고 요청을 보내면 
해당하는 Q&A 게시물 데이터를 반환합니다. 
만약 불러오기에 실패하면 실패처리를 합니다. 인가 실패, 데이터베이스 에러가 발생할 수 있습니다.
...
| name | type | description | required |
|---|:---:|:---:|:---:|
| searchWord | int | 접수 번호 | 0 |

###### Example
```bash
curl -v -X GET POST "http://localhost:4000/api/vi/board/{receptionNumber}" \
 -H "Authorization: Bearer {JWT}"
```
...
| writeDatetime | String | 작성일</br>(yyyy.mm.dd 형태) | 0 |
| viewCount | int | 조회수 | 0 |
...
Example
curl -v -X PATCH "http://localhost:4000/api/vi/board/{receptionNumber}/increase-view-count$" \
 -H "Authorization: Bearer {JWT}"
Response
Header
name	description	required
...
Example
curl -v -X POST "http://localhost:4000/api/vi/board/${receptionNumber}/comment" \
 -H "Authorization: Bearer {JWT}" \
 -d "comment={commnet}"
Response
Header
